"""Deterministic materialization of an extract into the exact input CSVs and
golden answer key the harness needs.

Vendored from agents/tools/materialize_golden.py (ticket 19; map invariant:
never import from agents/). Near-verbatim: the CLI main() is dropped -- the
studio invokes the functions from the materializer stage -- and the module
docstring is retitled; every jail, rung rule and byte contract ports whole.
...
go under ``golden/``. Each CSV is written with the delimiter the exploder
SNIFFED for that source (recorded in extract provenance) -- so the fixture
keeps the real file's separator and the configurator's reader round-trips it
-- falling back to ``;`` only for a table/transcribed source that has no
source file. All CSVs are RFC-4180 double-quoted (the configurator pairs this
with ``csv_option: true``) so a value containing the separator round-trips
...
def _sep_for(name, extract):
    ...
    extract provenance for that source, else ``_DEFAULT_SEP``. This keeps the written
...
def _safe_name(name: str) -> str:
    ...
    The name comes verbatim from a requirements-doc H2 subheading (or, on the
    typed door, an attachment's basename), so a crafted input could set it to an
    absolute path, ``..``, or a path with separators and write the materialized
    CSV outside the work dir. This mirrors run_and_validate's frozen path-jail:
    require a single safe filename component, else raise (the whole materialize
    refuses, before any harness run or human gate)."""
    if (not name) or name in (".", "..") or os.path.isabs(name) or os.path.basename(name) != name:
        raise ValueError(
            f"unsafe source/output name from the requirements input "
...
def materialize_expected(extract: dict, work_dir) -> dict:
    ...
    key is present (the normalizer path and the studio's typed door), an output is
    graded ONLY if its rung is "1" or "2". A rung-3 (3a/3b) or LLM-authored output,
    a missing provenance entry, or any unknown/``needs_human`` token all fall to
    ``graded: false`` -- so no transcribed answer key ever lands on disk as
    gradable, independent of any LLM. The template path (no ``provenance`` key)
    is unchanged: ``graded`` stays ``len(rows) > 0``."""
