...
signals, and diff actual output against golden expected data.

Vendored from ``agents/tools/run_and_validate.py`` by ticket 19 (map
invariant: vendor, never import ``agents/``). The studio invokes this file's
CLI as a SUBPROCESS of the agent core -- job code cells are RCE-capable and
must never be able to take the core down (ticket 04's execution isolation);
the engine (``src.v1``) is imported strictly read-only. Studio addition,
marked below: ``diff_frames`` embeds up to ``_EXAMPLE_CAP`` offending keys
per diff bucket with expected-vs-actual values per differing column -- the
value-visible diagnostician's evidence (ticket 04) -- and ``check`` keeps its
``reasons`` lines compact so the examples live once, in the diffs.

This module runs a job config IN-PROCESS through ``ETLEngine`` (no further
subprocess) and harvests the run's signals into a ``RunResult``. The engine's
``execute()`` returns two different dict shapes -- a success shape (with
``global_map`` and ``job_aborted``) and an exception shape (which has
neither) -- so every field is read with ``.get()``. The ACTUAL output is read
back from the produced FILES, not from in-memory flows (which the engine
clears per subjob). Dropped components are the config ids whose component
``type`` is not registered -- the exact rule the fail-open engine uses to
silently skip a component (engine.py:192-194). This is NOT "absent from
``component_stats``": a known-type component that simply never ran (e.g. one
in an untriggered conditional subjob) is legitimately missing from
``component_stats`` yet is NOT dropped.
"""
...
# Studio addition (ticket 19): bounded per-bucket evidence for the
# value-visible diagnostician -- up to this many offending keys per diff
# bucket, each carrying expected-vs-actual per differing column.
_EXAMPLE_CAP = 5


def _key_repr(idx):
    """A key value as JSON-friendly content: scalar -> str, composite -> list of str."""
...
def _row_dict(series) -> dict:
    return {str(c): str(v) for c, v in series.items()}


def diff_frames(actual, expected, keys):
    """Diff an actual output frame vs expected: keyed diff, or bag equality if keys is None.

    Studio addition (ticket 19): every failure bucket carries an ``examples``
    entry -- up to ``_EXAMPLE_CAP`` offending keys with real values (missing
    keys with their expected row, unexpected keys with their actual row,
    mismatched keys with expected-vs-actual per differing column) -- so the
    diagnostician reads concrete evidence, not counts."""
    if actual is None:
        head = expected.head(_EXAMPLE_CAP)
        return {"equal": False, "missing": int(len(expected)), "unexpected": 0, "value_mismatch": 0,
                "expected_rows": int(len(expected)), "actual_rows": 0,
                "examples": {"missing": [
                    {"key": ([str(row[k]) for k in keys] if keys else None),
                     "expected_row": _row_dict(row)}
                    for _, row in head.astype(str).iterrows()]},
    ...
    if not keys:
        ...
        examples = {}
        if a != e:
            only_e = [t for t in e if t not in a][:_EXAMPLE_CAP]
            only_a = [t for t in a if t not in e][:_EXAMPLE_CAP]
            examples = {"only_in_expected": [dict(zip(cols, t)) for t in only_e],
                        "only_in_actual": [dict(zip(cols, t)) for t in only_a]}
    ...
    if missing_key_cols:
        return {"equal": False, "missing": int(len(expected)), "unexpected": 0, "value_mismatch": 0,
                "expected_rows": int(len(expected)), "actual_rows": int(len(actual)),
    ...
    for idx in common:
        ...
        bad_cols = [c for c in cols if er.get(c) != ar.get(c)]
        if bad_cols:
            mismatch += 1
            if len(mismatch_examples) < _EXAMPLE_CAP:
                mismatch_examples.append({
                    "key": _key_repr(idx),
                    "columns": {c: {"expected": str(er.get(c)), "actual": str(ar.get(c))}
                                for c in bad_cols}})
    examples = {}
    if len(missing):
        examples["missing"] = [
            {"key": _key_repr(idx), "expected_row": _row_dict(exp.loc[[idx]].iloc[0])}
            for idx in list(missing)[:_EXAMPLE_CAP]]
    if len(unexpected):
        examples["unexpected"] = [
            {"key": _key_repr(idx), "actual_row": _row_dict(act.loc[[idx]].iloc[0])}
            for idx in list(unexpected)[:_EXAMPLE_CAP]]
    if mismatch_examples:
        examples["value_mismatch"] = mismatch_examples
...
def check(run_result, expected, output_map, keys, output_types=None) -> dict:
    ...
    for name, exp_df in expected.items():
        ...
        if not d.get("equal", False):
            # Studio addition (ticket 19): keep the reason line compact --
            # the bounded examples live once, in the output's diff entry.
            bits = [f"{bucket}={d[bucket]}"
                    for bucket in ("missing", "unexpected", "value_mismatch")
                    if d.get(bucket)]
            bits += [f"{bucket}={d[bucket]}"
                     for bucket in ("unexpected_columns", "missing_columns")
                     if d.get(bucket)]
            if d.get("reason"):
                bits.append(str(d["reason"]))
            reasons.append(f"output {name!r} differs: " + ("; ".join(bits) or "content mismatch"))
