...
Vendored from agents/tools/normalize_validate.py (ticket 17; map invariant:
never import from agents/). The merge/guard/shape logic is word-for-word; the
structural deltas are the studio's:

- file CLI dropped -- the artifact bus owns files, so the entrypoint is
  ``validate_proposal(proposal, inventory, model_id)`` -> ``(status, payload)``
  with payload = the extract dict (ok | needs_human) or ``{"errors": [...]}``
  (shape_error). The conductor routes each status (shape_error -> ticket 16's
  shape-repair loop, needs_human -> the question channel) instead of exiting.
"""
...
from .extract_doc import compute_derived_facts
...
def _derive_rung(handle_id, inventory) -> str:
    ...
    if htype in ("embed", "sibling"):
        if ref.endswith((".xlsx", ".xls")):
            return "needs_human"  # no xlsx reader in the studio chain
...
def _reconcile_expected(rows, output_cols):
    """Reorder expected rows to the proposed output name-space if it reconciles; else keep the header and flag low-confidence (the expected side never hard-fails)."""
    if not output_cols:
        return list(rows), (
...
def _materialized_bytes(rows) -> bytes:
    """Serialize rows to the exact ';'-delimited CSV bytes the materializer would write (header = first row's keys)."""
    if not rows:
        return b""
...
# ------------------------------------------------------------------
# Tier: enforced deterministically at the grading boundary.
...
# ------------------------------------------------------------------
# Completeness cross-check: account for EVERY inventory handle.
# ------------------------------------------------------------------
        ...
        if (provenance.get(source) or {}).get("rung") in ("3a", "3b"):
            for col_fact in col_facts.values():
                ...
                if col_fact.get("max_group_size", 0) <= 1:
                    col_fact["max_group_size"] = 2  # raise the interpreter's non-unique-key ambiguity
    return facts


# ------------------------------------------------------------------
# assemble: tie the merge/guard helpers into the final extract dict.
# ------------------------------------------------------------------
...
def assemble(proposal, inventory, model_id="") -> tuple:
    """Integrate proposal+inventory into the final extract dict; return (extract_dict, extraction status)."""
    ...
    # ---- 3. Graded set + guards (rows > 0 is the graded line) ----
...
# normalizer proposal. Malformed-proposal failures route to the shape-repair
...
# ------------------------------------------------------------------
# validate_proposal: shape-validate, then assemble (the studio entrypoint).
# ------------------------------------------------------------------
def validate_proposal(proposal, inventory, model_id="") -> tuple:
    """Shape-validate then assemble a proposal dict.

    Returns ``(status, payload)``: on ``shape_error`` the payload is
    ``{"errors": [...]}`` (the normalizer feedback); on ``ok`` / ``needs_human``
    it is the assembled extract dict (whose ``extraction`` block carries the
    unaccounted/unresolved detail a needs_human question is composed from).
    """
    if not isinstance(proposal, dict):
        return "shape_error", {"errors": [{
            "pointer": "$",
            "why": f"proposal is not a JSON object (got {type(proposal).__name__})",
            "fix": "emit a well-formed proposal object with rules/sources_schema/located/coverage_map",
        }]}
    try:
        ...
        errors = [{
            "pointer": "$",
            "why": f"proposal has the wrong top-level shape: {exc}",
            "fix": "emit a well-formed proposal object with rules/sources_schema/located/coverage_map",
        }]
    if errors:
        logger.warning("[normalize_validate] shape_error: %d issue(s)", len(errors))
        return "shape_error", {"errors": errors}
    extract_dict, status = assemble(proposal, inventory, model_id)
    logger.info("[normalize_validate] status=%s tier=%s", status, extract_dict.get("tier"))
    return status, extract_dict
