...
Vendored from agents/tools/explode_doc.py (ticket 17; map invariant: never
import from agents/), with ticket 04's structural deltas:

- the purity scan is gone (docx_purity is dead -- every .docx takes the
...
extraction and the materializer are the two writers).
"""
...
from .extract_doc import _iter_block_items, _table_records
...
def _safe_name(name: str) -> str:
    """Fail-closed on an untrusted NAME before it becomes a filename (inlined
    from materialize_golden's frozen path-jail)."""
    if (not name) or name in (".", "..") or os.path.isabs(name) or os.path.basename(name) != name:
        raise ValueError(
            f"unsafe file name (must be a single filename component): {name!r}"
        )
    return name


def _jailed(root, fname: str) -> Path:
    """Resolve ``fname`` under ``root`` and confirm it stays inside root (defense
    in depth on top of _safe_name), so an extracted file can never escape."""
    root_real = Path(os.path.realpath(root))
    target = root_real / fname
    if not target.resolve().is_relative_to(root_real):
        raise ValueError(f"extraction path escapes the jail dir: {fname!r}")
    return target
...
def _safe_basename(member: str) -> str:
    ...
    return _safe_name(base)  # RAISES on empty/./.. via the fail-closed validator
...
# ------------------------------------------------------------------
# CSV dialect sniff + top-level explode().
# ------------------------------------------------------------------
