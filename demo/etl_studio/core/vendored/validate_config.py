...
Vendored from agents/tools/validate_config.py (ticket 17; map invariant:
never import from agents/); the CLI wrapper is dropped -- the configurator
stage runs ``validate_config`` as its bounded inner loop through a core-side
tool handler (subagent/terminal mechanics become core tools, ticket 11).
"""
from __future__ import annotations

import logging

from .component_schema import BASE_KEYS, IGNORED_KEYS, is_curated, load_schema, resolve_enum_ref
...
def validate_config(component_type: str, config: dict, strict: bool = True) -> list:
    ...
    enum_ref-backed schema. Every other registered engine component degrades
    gracefully to advisory-only (returns no errors); correctness for those
    falls to the engine's own ``_validate_config`` plus the oracle.
    """
