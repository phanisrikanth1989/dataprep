...
Vendored verbatim from agents/knowledge/landmines.py (ticket 17; map
invariant: never import from agents/). This is a ticket-11 knowledge SOURCE:
the studio renderer (core/knowledge.py) renders it at every core startup --
WITH the code anchors ticket 11's fit-pass restores -- and the slice map
serves it whole (doc-normalizer, interpreter, flow-designer) or
component-filtered via ``landmines_for`` (configurator, assembler,
diagnostician).
"""
