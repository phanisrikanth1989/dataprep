...
## Deferred to plan-3b (documented, not dropped)
Tolerance / bidirectional / netting / waterfall signals (Phases B-D); held-out sample; automated mutation-row generation; the full cardinality invariant battery (sum reconciliation, both-side accounting) and predicate-consistency on `break_code`; the duplicate-key-break golden job (needs a UniqueRow-to-break pattern). These build on Task 3-5 interfaces.
