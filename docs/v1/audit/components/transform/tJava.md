...
2. **ENG-JC-002**: Implement imports reading in engine (parse import statements, add to bridge class loader).
3. **ENG-JC-003**: Add die_on_error handling (catch exceptions, set ERROR_MESSAGE global, fire error triggers).
4. **TEST-JC-001**: Add engine unit tests for JavaComponent.

### Long-term (Optimization)

5. **ENG-JC-004/005**: Improve bridge sync to handle key deletions and avoid stale reads.
6. **STD-JC-001**: Replace f-string logger calls with lazy formatting.
