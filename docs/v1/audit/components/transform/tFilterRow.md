...
1. **Add string operator support** (ENG-FR-002) -- CONTAINS, MATCHES_REGEX, etc.
2. **Implement FUNCTION pre-transforms** (ENG-FR-003) -- LENGTH, ABS_VALUE, TRIM
3. **Fix string coercion** (ENG-FR-004/BUG-FR-002) -- Use proper type-aware comparisons
4. **Remove print() statements** (STD-FR-001) -- 17+ debug prints in production code
5. **Add engine unit tests** (TEST-FR-001)

### Long-term (Optimization)

1. **Implement tFilterRows multi-reject routing** (ENG-FR-007)
2. **Vectorize advanced condition evaluation** (PERF-FR-001)
