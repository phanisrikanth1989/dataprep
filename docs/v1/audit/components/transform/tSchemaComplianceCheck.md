...
3. **ENG-SCC-003 (P1)**: Implement Custom Defined mode (read CUSTOMER, CHECKCOLS from config)
4. **ENG-SCC-004 (P1)**: Add date validation (fast and strict modes)
5. **ENG-SCC-005 (P1)**: Implement SUB_STRING truncation
6. **ENG-SCC-006 (P1)**: Read ALL_EMPTY_ARE_NULL from config instead of hardcoding
7. **ENG-SCC-007 (P1)**: Implement IGNORE_TIMEZONE for date validation
8. **TEST-SCC-001 (P2)**: Add engine unit tests

### Long-term (Optimization)

9. **ENG-SCC-008-011 (P2-P3)**: Error code variety, message format, byte-length check, per-column empty-null
