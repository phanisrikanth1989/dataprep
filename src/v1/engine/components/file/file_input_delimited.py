...
try:
    ...
    if (self.execution_mode == ExecutionMode.HYBRID and
        ...
        return self._read_streaming(
            filepath, delimiter, encoding, header_rows,
...
def _read_batch(
    ...
    # Determine engine
    engine = 'python' if (
        footer_rows > 0 or use_regex or skiprows is not None
    ) else 'c'
    ...
    if not text_enclosure or len(text_enclosure) != 1:
        logger.warning(
            f"Component {self.id}: Invalid text_enclosure "
            f"'{text_enclosure}'. Disabling quoting."
        )
        quote_params = {
    ...
    logger.info(
        f"Component {self.id}: Read {rows_read} rows from {filepath}"
    )

    # Log data types for all columns
    dtypes_info = {col: str(dtype) for col, dtype in df.dtypes.items()}
    logger.info(
        f"Component {self.id}: column dtypes: {dtypes_info}"
    )
...
def _read_streaming(
    ...
    logger.info(
        f"Component {self.id}: Reading file in streaming mode "
