    ...
    def _process(self, input_data: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if mode == 'include':
                ...
                if missing_columns:
                    logger.warning(f"Component {self.id}: Columns not found: {missing_columns}")

                if not columns_to_keep:
                    logger.warning(f"Component {self.id}: No valid columns to keep, returning empty DataFrame")
                ...
                if not columns_to_keep:
                    logger.warning(f"Component {self.id}: All columns excluded, returning empty DataFrame")
                    self._update_stats(total_rows, 0, total_rows)
                    return {'main': pd.DataFrame()}
            ...
            logger.info(f"Component {self.id}: " 
                        f"Input columns: {len(available_columns)}, "
                        f"Output columns: {len(result_df.columns)}")
