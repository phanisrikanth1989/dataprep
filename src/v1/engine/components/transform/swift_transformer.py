...
class SwiftTransformer(BaseComponent):
    ...
    def _init_transformer_config(self):
        ...
        if self.transform_config:
            # Extract key configuration sections only if config is already loaded
            ...
            # Initialize empty structures - will be populated at execution time
            self.input_fields = []
            self.output_fields = []
            self.output_layout = []
            self.field_mappings = {}
    ...
    def _ensure_config_loaded(self):
        ...
        if self.transform_config is None:
            if self.config_file:
                # Load external config now that context variables are available
    ...
    def _load_lookup_files(self):
        ...
        for lookup in self.lookups_config:
            ...
            try:
                ...
                # Use the resolved path directly (should already be absolute from config)
    ...
    def _load_external_config(self, config_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            resolved_config_path = os.path.normpath(resolved_config_path)
