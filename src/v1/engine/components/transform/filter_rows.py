...
class FilterRows(BaseComponent):
    ...
    def _process(self, input_data: pd.DataFrame = None):
        ...
        logger.debug(f"FilterRows[{self.id}] - Starting _process with input_data shape: {getattr(input_data, 'shape', None)}")
        ...
        try:
            ...
            print(f"[FilterRows] use_advanced: {use_advanced}, "f"logical_operator: {logical_operator}, "f"conditions: {conditions}")
            ...
            if use_advanced and advanced_condition:
                logger.debug(f"FilterRows[{self.id}] - Using advanced condition: {advanced_condition}")
                ...
                logger.debug(f"FilterRows[{self.id}] - Using simple conditions: {conditions} "f"with logical_operator: {logical_operator}")
                masks = []
                ...
                for cond in conditions:
                    ...
                    if col not in input_data.columns:
                        logger.warning(f"FilterRows[{self.id}] - Column '{col}' not found in input data.")
                    ...
                    if op == '==':
                        ...
                        logger.warning(f"FilterRows[{self.id}] - Unsupported operator '{op}'.")
                    ...
                    for idx, v in enumerate(col_data):                    
                        print(f"[FilterRows] Row {idx}: {repr(v)} {op} {repr(val_stripped)} => {repr(mask.iloc[idx])}")
                ...
                if masks:
                    if logical_operator == 'AND':
                        ...
                        logger.warning(f"FilterRows[{self.id}] - Unknown logical operator '{logical_operator}', defaulting to AND.")
                        print(f"[FilterRows] Unknown logical operator '{logical_operator}', defaulting to AND.")
                        final_mask = masks[0]
                        for m in masks[1:]:
                            final_mask = final_mask & m
            ...
            print(f"[FilterRows] Accepted shape: {accepted.shape}, "f"Rejected shape: {rejected.shape}")
            self._update_stats(len(input_data), len(accepted), len(rejected))
            logger.debug(f"FilterRows[{self.id}] - Accepted rows: {len(accepted)}, "f"Rejected rows: {len(rejected)}")
            return {'main': accepted, 'reject': rejected}
        except Exception as e:
            logger.error(f"FilterRows[{self.id}] - Error during filtering: {e}")
    ...
    def validate_config(self) -> bool:
        ...
        if config.get('use_advanced', False):
            if not config.get('advanced_condition'):
                logger.error(f"FilterRows[{self.id}] - advanced_condition required when use_advanced is True.")
                return False
        else:
            if not config.get('conditions') or not isinstance(config.get('conditions'), list):
                logger.error(f"FilterRows[{self.id}] - conditions must be a non-empty list.")
                return False
        return True
