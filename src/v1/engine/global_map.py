...
class GlobalMap:
    ...
    Used for sharing data between components and tracking execution stats.
    """
    ...
    def remove(self, key: str) -> None:
        ...
        if key in self._map:
            del self._map[key]
            logger.debug(f"GlobalMap: Removed {key}")
    ...
    def put_component_stat(self, component_id: str, stat_name: str, value: int) -> None:
        """Store component statistics (NB_LINE, NB_LINE_OK, NB_LINE_REJECT)"""
    ...
    def __repr__(self) -> str:
        return f"GlobalMap(items={len(self._map)}, components={len(self._component_stats)})"