...
- **D-14:** Per-row error in `python_row_component` routes the offending input row to the `reject` output DataFrame with two appended columns: `errorMessage` (str, the exception message) and `errorCode` (int, default 1). Matches the engine's existing tFilterRow REJECT flow established in Phase 7.1. For `java_row_component` (v1 limitation per Q2 Option A, RESEARCH.md Pitfall #2): the existing `JavaBridge.executeJavaRow` protocol is all-or-nothing (`JavaBridge.java:243-246`) and per D-19 the bridge protocol is locked. On any per-row exception the entire input batch is therefore routed to `reject` with `errorMessage` carrying the bridge exception message and `errorCode=1` on every row. Finer-grained per-row REJECT for tJavaRow is deferred to a future BRDG-* phase that adds an `executeJavaRowWithReject` variant. Talend's native tJavaRow has no REJECT flow at all (verified Talaxie tJavaRow_java.xml has no DIE_ON_ERROR), so even the batch-level reject is a DataPrep enhancement -- v1 ships with reduced fidelity for the Java side; full per-row fidelity for tJavaRow is a future phase. Both variants continue processing unless `die_on_error=True` (D-15).
