...
Enums NOT engine-validated (validate_config only checks name/column/expression presence + java bridge): join_mode {LEFT_OUTER_JOIN,INNER_JOIN}; matching_mode {UNIQUE_MATCH,FIRST_MATCH,ALL_MATCHES,ALL_ROWS}; lookup_mode {LOAD_ONCE,RELOAD,CACHE_OR_RELOAD}; operator {"="} only.
