"""Unit tests for OracleRollback engine component.

Mock-based: registration, structural validation, rollback + optional close,
...
def _make_component(config, oracle_manager=None, global_map=None):
    from src.v1.engine.components.database.oracle_rollback import OracleRollback

    gm = global_map if global_map is not None else GlobalMap()
    comp = OracleRollback(
        component_id="tOracleRollback_1",
        config=config,
        global_map=gm,
...
class TestRegistration:
    def test_both_aliases_resolve(self):
        ...
        from src.v1.engine.components.database.oracle_rollback import (
            OracleRollback,
        )

        assert REGISTRY.get("OracleRollback") is OracleRollback
        assert REGISTRY.get("tOracleRollback") is OracleRollback
