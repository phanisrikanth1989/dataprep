"""Tests for ReplicateConverter (tReplicate -> v1 Replicate config)."""
...
def _make_node(params=None, schema=None, component_id="rep_1",
...
class TestNeedsReview:
    ...
    def test_needs_review_count(self):
        """Exactly 2 needs_review entries: output_count, die_on_error."""
        node = _make_node()
        result = ReplicateConverter().convert(node, [], {})
    ...
    def test_needs_review_output_count(self):
        """One entry mentions output_count."""
        node = _make_node()
        result = ReplicateConverter().convert(node, [], {})
        issues = [e["issue"] for e in result.needs_review]
        assert any("output_count" in issue for issue in issues)

    def test_needs_review_die_on_error(self):
        """One entry mentions die_on_error."""
        node = _make_node()
        result = ReplicateConverter().convert(node, [], {})
        issues = [e["issue"] for e in result.needs_review]
        assert any("die_on_error" in issue for issue in issues)
...
class TestComponentStructure:
    ...
    def test_has_type(self):
        node = _make_node()
        result = ReplicateConverter().convert(node, [], {})
        assert result.component["type"] == "Replicate"

    def test_has_original_type(self):
        node = _make_node()
        result = ReplicateConverter().convert(node, [], {})
        assert result.component["original_type"] == "tReplicate"
    ...
    def test_has_id(self):
        node = _make_node(component_id="rep_1")
        result = ReplicateConverter().convert(node, [], {})
        assert result.component["id"] == "rep_1"
