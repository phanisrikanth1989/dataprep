"""Tests for OracleCloseConverter (tOracleClose -> v1 oracle close config)."""
...
def _make_node(params=None, schema=None, component_id="oc_1",
               component_type="tOracleClose"):
    """Create a TalendNode for tOracleClose testing."""
    return TalendNode(
...
class TestRegistration:
    ...
    def test_registered_in_registry(self):
        assert REGISTRY.get("tOracleClose") is OracleCloseConverter
...
class TestDefaults:
    ...
    def test_connection_default(self):
        node = _make_node()
        result = OracleCloseConverter().convert(node, [], {})
...
class TestNeedsReview:
    ...
    def test_needs_review_message(self):
        node = _make_node()
        result = OracleCloseConverter().convert(node, [], {})
        assert "No concrete engine implementation for tOracleClose" in result.needs_review[0]["issue"]
...
class TestComponentStructure:
    ...
    def test_id_at_top_level(self):
        node = _make_node()
        result = OracleCloseConverter().convert(node, [], {})
        assert result.component["id"] == "oc_1"

    def test_type_at_top_level(self):
        node = _make_node()
        result = OracleCloseConverter().convert(node, [], {})
        assert result.component["type"] == "tOracleClose"

    def test_original_type_at_top_level(self):
        node = _make_node()
        result = OracleCloseConverter().convert(node, [], {})
        assert result.component["original_type"] == "tOracleClose"
