"""Tests for FlowToIterateConverter (tFlowToIterate -> v1 iterate config)."""
import xml.etree.ElementTree as ET

from src.converters.talend_to_v1.components.base import SchemaColumn, TalendNode
from src.converters.talend_to_v1.components.iterate.flow_to_iterate import FlowToIterateConverter
...
def _make_node(params=None, schema=None, component_id="fti_1",
...
def _make_map_data(rows):
    """Generate MAP TABLE data with stride-2: KEY + VALUE per row.
...
class TestRegistration:
    ...
    def test_registered_in_registry(self):
        """tFlowToIterate is registered in the converter registry."""
        assert REGISTRY.get("tFlowToIterate") is FlowToIterateConverter


class TestDefaults:
    """Verify all parameters have correct defaults when no params provided."""

    def test_default_map_default_true(self):
        """DEFAULT_MAP defaults to True when absent."""
        node = _make_node()
        result = FlowToIterateConverter().convert(node, [], {})
        assert result.component["config"]["default_map"] is True

    def test_map_entries_default_empty(self):
        """map_entries defaults to empty list when no MAP param."""
        node = _make_node()
        result = FlowToIterateConverter().convert(node, [], {})
...
class TestParameterExtraction:
    ...
    def test_default_map_false(self):
        """DEFAULT_MAP=false is correctly extracted."""
        node = _make_node(params={"DEFAULT_MAP": "false"})
        result = FlowToIterateConverter().convert(node, [], {})
...
class TestTableParsing:
    """Verify MAP TABLE parameter parsing."""

    def test_map_table_parsed(self):
        """MAP table with KEY/VALUE entries parsed into list of dicts."""
        map_data = _make_map_data([
            ('"col_a"', '"var_a"'),
            ('"col_b"', '"var_b"'),
        ])
        node = _make_node(params={"DEFAULT_MAP": "false", "MAP": map_data})
        result = FlowToIterateConverter().convert(node, [], {})
        entries = result.component["config"]["map_entries"]
        assert len(entries) == 2
        assert entries[0] == {"key": "col_a", "value": "var_a"}
        assert entries[1] == {"key": "col_b", "value": "var_b"}

    def test_map_table_empty_when_missing(self):
        """No MAP param produces empty list."""
        node = _make_node()
        result = FlowToIterateConverter().convert(node, [], {})
        assert result.component["config"]["map_entries"] == []

    def test_map_table_incomplete_stride_skipped(self):
        """Odd number of entries means incomplete group is dropped."""
        data = [
            {"elementRef": "KEY", "value": '"col_a"'},
            {"elementRef": "VALUE", "value": '"var_a"'},
            {"elementRef": "KEY", "value": '"col_b"'},
            # Missing VALUE for col_b -- incomplete stride
        ]
        node = _make_node(params={"DEFAULT_MAP": "false", "MAP": data})
        result = FlowToIterateConverter().convert(node, [], {})
        entries = result.component["config"]["map_entries"]
        assert len(entries) == 1
        assert entries[0] == {"key": "col_a", "value": "var_a"}

    def test_map_table_values_strip_quotes(self):
        """TABLE values like '"col1"' are stripped to 'col1'."""
        map_data = _make_map_data([('"my_key"', '"my_value"')])
        node = _make_node(params={"DEFAULT_MAP": "false", "MAP": map_data})
        result = FlowToIterateConverter().convert(node, [], {})
        entries = result.component["config"]["map_entries"]
        assert entries[0]["key"] == "my_key"
        assert entries[0]["value"] == "my_value"
...
class TestComponentStructure:
    ...
    def test_has_id_at_top_level(self):
        ...
        node = _make_node(component_id="fti_1")
        result = FlowToIterateConverter().convert(node, [], {})
        assert result.component["id"] == "fti_1"

    def test_has_type_at_top_level(self):
        """Component dict has 'type' at top level."""
        node = _make_node()
        result = FlowToIterateConverter().convert(node, [], {})
        assert result.component["type"] == "tFlowToIterate"

    def test_has_original_type_at_top_level(self):
        """Component dict has 'original_type' at top level."""
        node = _make_node()
        result = FlowToIterateConverter().convert(node, [], {})
        assert result.component["original_type"] == "tFlowToIterate"
